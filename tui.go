	...
	go func() {
		for {
			...
			case evt, ok := <-eventCh:
				...
				p.Send(mcpDashboardMsg{evt: evt})
			}
		}
	}()
}
...
func (m mcpDashboardModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		m.width = msg.Width
		m.height = msg.Height
		// Update stored table height for accurate page scrolling
		tableH := msg.Height - 4 - inspectorRows
		if tableH < 3 {
			tableH = 3
		}
		d.mu.Lock()
...
func renderSetupScreen(width, height int, state dashboardSnapshot, endpoint string) string {
	...
	b.WriteString(fmt.Sprintf("  2. Add header:      %s\n", setupValue.Render("Authorization: Bearer "+state.AuthToken)))
...
func renderTableBody(width, rows int, state dashboardSnapshot, filtered []int) string {
	// nil filtered means "all events" (no filter active)
	total := len(filtered)
	if filtered == nil {
	...
	vp := clampViewport(selFilterIdx, state.Viewport, rows, total)
	end := vp + rows
	if end > total {
		end = total
	}
...
// filterEvents returns indices of events matching the filter.
// Returns nil when filter is empty (caller treats nil as "all events").
...
func (d *MCPDashboard) recordEvent(evt MCPEvent) {
	...
	// Count (always, regardless of paused state)
	d.reqCount++
	...
	// Update selection/viewport for live mode
	if evicted {
		if d.selected > 0 {
			d.selected--
		}
...
func wrapText(value string, maxW int) []string {
	...
	for i := 1; i < len(tokens); i++ {
		...
		if runeWidth(current)+1+runeWidth(n) > maxW {
			lines = append(lines, fitToWidth(current, maxW))
			current = n
			continue
		}
		current = current + " " + n
